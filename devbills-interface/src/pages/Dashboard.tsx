...
const Dashboard = () => {
  ...
  const [montlyItemsData, setMontlyItemsData] = useState<MonthlyItem[]>([]);
  ...
  const renderPieChartLabel = ({ categoryName, percent }: ChartLabelProps): string | undefined => {
    if (percent === undefined || categoryName === undefined) return;
            ...
            {montlyItemsData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={montlyItemsData} margin={{ left: 40 }}>
                  ...
                  <Bar dataKey="income" name="Despesas" fill="#37E359" />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex items-center justify-center h-64 text-gray-500">
